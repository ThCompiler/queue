...
test:test('tasks by state', function(test)
    ...
    local ready, taken = 0, 0
    for _, task in state_tube.raw:tasks_by_state(state.READY) do
        if task[2] == state.READY then ready = ready + 1 end
    end
    for _, task in state_tube.raw:tasks_by_state(state.TAKEN) do
        if task[2] == state.TAKEN then taken = taken + 1 end
    end

    test:is(ready, 6, 'ready task count')
    test:is(taken, 4, 'taken task count')
end)
